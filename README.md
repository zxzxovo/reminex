...
- **🌐 Web 界面**：现代化的 Web 搜索界面，支持多数据库选择、关键词高亮、自定义分隔符等高级功能
...
Reminex 提供了功能丰富的现代化 Web 界面，支持以下高级功能：
...
# 启动多数据库 Web 服务（支持切换）
reminex web -d db1.reminex.db -d db2.reminex.db -d db3.reminex.db
```

#### 多数据库选择
- 使用 Material Design 风格的 Chip 组件展示数据库列表
- 支持单选、多选或全选数据库进行搜索
- 可同时在 A、B、C 数据库中搜索并合并结果
- 默认选中第一个数据库，如无数据库则不显示

#### 关键词高亮
- 搜索结果中的匹配关键词会自动高亮显示
- 支持自定义高亮颜色（默认为橙红色 #ff7043）
- 可使用颜色选择器或手动输入颜色值（支持 #HEX、rgb()、hsl() 等格式）
- 高亮设置会保存到浏览器本地存储
...
#### 其他 Web 功能
- 🎨 **主题切换**：深色/浅色/自动模式，支持跟随系统
- 📏 **紧凑度调整**：宽松/标准/紧凑三种显示模式
- 📊 **树形展示**：层级目录树状显示搜索结果
- 🔍 **高级搜索**：仅搜索文件名、区分大小写、结果数量限制
- 📝 **搜索历史**：自动保存搜索记录，可快速重复搜索
- 💾 **结果导出**：支持 JSON 和 CSV 格式导出搜索结果
- 📱 **响应式设计**：完美适配桌面和移动设备
